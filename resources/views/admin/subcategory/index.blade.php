  ...
  <div class="br-section-wrapper">
    <div class="row">
      <div class="col-6">
        ...
        <h6 class="br-section-label">Sub Categories List</h6>
        <p class="br-section-text"></p>
      </div>
      <div class="col-6">
        <a href="{{ route('admin.subcategories.create') }}" class="btn btn-primary float-right">Add New</a>
      </div>
    </div>
              ...
              <tbody>
                ...
                <tr>
                 ...
                 {{--  <th > @if($row->image)<img src="{{ asset($row->image) }}" width="80" alt=""> @endif</th> --}}
                  ...
                  <th >{{ $row->parent != null ? $row->parent->name : null  }}</th>
                  <td>{{ $row->created_at->format('d M Y') }}</td>
                  <td>@if($row->status==1) <p class="text-success">Active</p>
                   @else <p class="text-danger">Deactivateed</p> @endif
                 </td>
                  <td class="text-center">
                    <a href="{{ route('admin.subcategories.edit',$row->id) }}" class="btn btn-outline-secondary btn-sm">Edit</a>
                    <a href="javascript:viod()" class="btn btn-outline-danger btn-sm ml-2" onclick="archiveFunction('{{ $row->id }}');">Delete</a>
                    <form id="delete-form{{ $row->id }}" action="{{ route('admin.subcategories.destroy',$row->id) }}" method="POST">@csrf </form>
                  </td>
                </tr>
                @endforeach
                @endif
