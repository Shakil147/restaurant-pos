  ...
  <div class="br-section-wrapper">
    <div class="row">
      <div class="col-6">
        ...
        <h6 class="br-section-label text-capitalize">staff List</h6>
        <p class="br-section-text"></p>
      </div>
      <div class="col-6">
        <a href="{{ route('admin.staff.create') }}" class="btn btn-primary float-right">Add New</a>
      </div>
    </div>
      ...
      <table class="table mg-b-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <th >{{ $row->phone }}</th>
            <th >{{ $row->type ? $row->type->name : null }}</th>
            <td>@if($row->status==1) <p class="text-success">Active</p>
             @else <p class="text-danger">Deactivateed</p> @endif
           </td>
            <td class="text-center">
              <a href="{{ route('admin.staff.edit',$row->id) }}"class="btn btn-outline-secondary btn-sm ml-2" >Edit</a>
              <a href="javascript:viod()" class="btn btn-outline-danger btn-sm ml-2" onclick="archiveFunction('{{ $row->id }}');">Delete</a>
              <form id="delete-form{{ $row->id }}" action="{{ route('admin.staff.destroy',$row->id) }}" method="POST">@csrf </form>
            </td>
          </tr>
          @endforeach
          @endif
