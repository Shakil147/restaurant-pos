...
<div class="br-pagetitle">
  ...
  <div>
    <h4 class="text-capitalize">items List</h4>
    <p class="mg-b-0"></p>
  </div>
</div>
  ...
  <div class="br-section-wrapper">
    <div class="row">
      <div class="col-6">
        ...
        <h6 class="br-section-label text-capitalize">items List</h6>
        <p class="br-section-text"></p>
      </div>
      <div class="col-6">
        <a href="{{ route('admin.items.create') }}" class="btn btn-primary float-right">Add New</a>
      </div>
    </div>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
              <a href="{{ route('admin.items.edit',$row->id) }}"class="btn btn-outline-secondary btn-sm ml-2" >Edit</a>
              <a href="javascript:viod()" class="btn btn-outline-danger btn-sm ml-2" onclick="archiveFunction('{{ $row->id }}');">Delete</a>
              <form id="delete-form{{ $row->id }}" action="{{ route('admin.items.destroy',$row->id) }}" method="POST">@csrf </form>
            </td>
          </tr>
          @endforeach
          @endif
